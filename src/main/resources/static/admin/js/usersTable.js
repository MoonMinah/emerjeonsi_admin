        ...
        .then(response => {
            ...
            if(tableBody) {
                ...
                users.forEach(user => {
                    ...
                    banBtn.classList.add("ban-button");
                    banBtn.addEventListener("click", ((userNo) => {
                        return () => banUser(userNo);
                    })(user.userNo));
...
function banUser(userNo) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            // axios를 통해 강퇴 처리 API 호출
            axios.post(`/api/admin/users/ban`, { userNo })
                .then(() => {
                    Swal.fire("완료", "회원이 성공적으로 강퇴되었습니다.", "success");
                    fetchUsers(currentPage, pageSize); // 테이블 새로고침
                })
                .catch((error) => {
                    console.error("오류 발생:", error.response || error.message);
                    Swal.fire("오류", "강퇴 처리 중 문제가 발생했습니다.", "error");
                });
        }
    });
}
